        ...
        issues: [
          {
            tag: "지도 한 번에 쿼리 수천 번",
            problem:
              "핀 목록을 가져온 뒤 핀마다 후기를 다시 묻고, 후기마다 사진을 또 물었다. 목록 하나에 질문이 줄줄이 따라붙는 N+1이다. 핀 200개면 쿼리가 천 번을 넘었다.",
            solution:
              "핀과 후기와 사진을 조인 한 번으로 함께 가져오고, 누적 수거량은 데이터베이스가 직접 더하게 했다. 핀이 몇 개든 쿼리 수가 늘지 않는다.",
          },
          {
            tag: "엑셀 한 장에 메모리 세 벌",
            problem:
              "시트 전체를 메모리에 만든 뒤 그것을 다시 바이트 배열로 옮겨 담았다. 같은 데이터를 세 벌 쥐고 있어서 후기가 쌓일수록 메모리가 세 배로 늘었다.",
            solution:
              "행을 만드는 대로 내보내고 쓴 행은 버리게 바꿨다. 메모리에는 몇백 행만 남아서, 만 건이든 십만 건이든 쓰는 양이 같다.",
          },
          {
            tag: "매번 처음부터 훑던 조회",
            problem:
              "로그인마다 이메일로 회원을 찾고 대시보드마다 전체 후기를 더했는데, 찾는 기준에 인덱스가 없어 테이블을 끝까지 읽었다. 행이 늘면 그만큼 느려졌다.",
            solution:
              "자주 찾는 기준에 인덱스를 만들어 훑지 않고 바로 짚게 했다. 자주 보지만 잘 바뀌지 않는 누적 통계는 Redis에 얹어 같은 집계를 반복하지 않게 했다.",
          },
        ],
        repo: "https://github.com/koreamax/piudaback",
        status: "종료",
