...
export const categories: Category[] = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        problem:
          "지도를 한 번 여는 데 핀마다 후기를, 후기마다 사진을 따로 물어보느라 쿼리가 수천 번 나갔다. 엑셀 내려받기는 시트 전체를 메모리에 올린 뒤 다시 바이트 배열로 옮겨 담아 같은 데이터를 세 벌씩 쥐고 있었고, 자주 거는 조회에는 인덱스가 없었다.",
        solution:
          "핀과 후기, 사진을 한 번의 조인으로 끌어오고 합계는 집계 쿼리에 맡겨 요청당 쿼리 수를 고정했다. 엑셀은 행을 만드는 족족 흘려보내 메모리가 건수와 무관하게 일정하도록 바꿨다. 조회 조건에 맞춰 인덱스를 잡고, 매번 다시 세던 누적 통계는 Redis에 올려 원본을 건드리지 않게 했다.",
        repo: "https://github.com/koreamax/piudaback",
        status: "종료",
      },
    ],
